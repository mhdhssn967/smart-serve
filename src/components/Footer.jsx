...
const CartIcon = ({ active, count }) => (
  ...
  return (
    <div>
      {/* ── BOTTOM NAV ── */}
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w bg-white/95 backdrop-blur border-t border-gray-100 shadow-lg z-40 flex justify-around px-0 pt-2.5 pb-6">
        {[
        ...
        {cartCount > 0 && nav !== "cart" && (
          <Link to={'/cart'}><div
            onClick={() => setNav("cart")}
            className="pop absolute -top-14 left-4 right-4 bg-orange-500 rounded-2xl px-4 py-3 flex justify-between items-center cursor-pointer shadow-lg shadow-orange-200"
          >
